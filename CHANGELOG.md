...
**FROM**：照妖镜 · 镜子反射分歧
**TO**：FindGap · 下游视角找 gap + 给优化方案 + 三层证据兜底

### 核心变更

- 产品名从"照妖镜"改为 **FindGap**
- 新增触发词 `/findgap` `/fg`，保留 `/照` `/照妖镜` 兼容
- 输出格式从"多角色碰撞映射"改为 **Flat List 单一声音直述**
- 去掉所有用户可见的规则代号（11 条规则仍在内部驱动检测）
- 每条 gap 必须包含 `原文：` 引用
- 🔴 和 🟠 必须包含 `优化方案：` 含决策依据
- 三层证据体系：Tier 1 搜索验证 → Tier 2 上下文推理兜底 → Tier 3 信息不足标注
- 红线从"不给建议"改为"优化方案给方向不给结论"
...
- `dogfood/conventions.md`：新记录格式
- `CONTRIBUTING.md`：FindGap 框架
- `ROADMAP.md`：v0.9 方向
