...
> 场景：评审 PRD《MisPOS 1.0.38 · 离线模式下聚合码支付补单》。
...
🔴 P0 × 2  🟡 P1 × 3  🟢 P2 × 1
Top 风险：
1. 🔴 OfflinePayQueue 未做幂等键，断网重试将重复扣款
2. 🔴 对账模块未覆盖"支付成功但补单失败"分支，资损无兜底
3. 🟡 PRD §4.2 与状态机 §6.1 对"超时窗口"定义冲突（30s vs 60s）
下一步：今天 18:00 前由 @lihu 修订 PRD §4.2 并补充幂等键设计，再发起二评。
```

#### 详细版

##### Finding #1 · 🔴 P0 · 离线补单队列缺少幂等键，存在重复扣款风险

- **定位**：`docs/prd/1.0.38-offline-pay.md:142:1` 与 `app/src/main/java/.../OfflinePayQueue.kt:88:5`
- **命中三法**：
  - [x] 反证法：假设网络在"支付网关已扣款 / ACK 未回 POS"瞬间断开，重试将再次提交同一笔
  - [x] 对照法：对照 1.0.35 收银台主链路（已用 `clientTradeNo` 做幂等），离线链路缺失同款字段
  - [x] 边界法：弱网 + 队列堆积 ≥ 2 条 + 应用被系统杀死后重启
- **证据**：
  ```
  PRD §5.3："失败任务自动重试，最多 3 次"
  代码 OfflinePayQueue.kt:88  retry(task) { gateway.pay(task.amount, task.orderId) }
  // 注意：orderId 在重试链路里会被重新生成
  ```
- **影响**：单笔资损 = 客单价 × 重试次数；按 1.0.37 离线日均 1.2 万笔估算，潜在日资损上限 ~36 万元
- **建议**：在 `OfflinePayTask` 增加 `idempotencyKey = sha256(orderId + terminalId + ts)`，网关侧建唯一索引
- **复查命令**：
  ```bash
  rg -n "idempotencyKey|clientTradeNo" app/src/main/java/.../offline/
  rg -n "retry\\(" app/src/main/java/.../OfflinePayQueue.kt
  ```
- **误报申诉**：若网关侧已在服务端做了 `outTradeNo` 去重，请回复 `DROP: 服务端已去重，附接口契约链接`。

##### Finding #2 · 🟡 P1 · 超时窗口定义在 PRD 与状态机文档间冲突

- **定位**：`docs/prd/1.0.38-offline-pay.md#4.2-timeout` vs `docs/design/pay-state-machine.md#6.1`
- **命中三法**：
  - [x] 反证法：开发按 PRD 实现 30s、测试按状态机断言 60s，必然产生用例失败
  - [ ] 对照法：—
  - [x] 边界法：恰好在 30s~60s 之间返回的网关响应会被两边判定为不同状态
- **证据**：PRD 写 "30s 未回 ACK 视为失败"；状态机文档写 "PENDING → FAIL 触发条件：60s 无响应"
- **影响**：状态错乱导致补单逻辑被错误触发，间接放大 Finding #1 的资损面
- **建议**：以网关 SLA（实测 P99 = 45s）为锚，统一为 60s，并在 PRD §4.2 显式引用状态机文档
- **复查命令**：
  ```bash
  rg -n "30s|60s|timeout" docs/prd/1.0.38-offline-pay.md docs/design/pay-state-machine.md
  ```
